...
beforeAll(async () => {
  ...
  harness.config = {
    ...
    metricsAuthToken: "metrics-test-token-that-is-at-least-32-characters",
...
describe("realtime gateway executable HTTP and upgrade boundary", () => {
  it("dispatches liveness, readiness, and unknown HTTP routes", async () => {
    ...
    expect(unauthenticatedMetrics.headers.get("www-authenticate")).toBe("Bearer");

    const authorizedMetrics = await fetch(`http://127.0.0.1:${port}/metrics`, {
      headers: { authorization: "Bearer metrics-test-token-that-is-at-least-32-characters" },
    });
    expect(authorizedMetrics.status).toBe(200);
    await expect(authorizedMetrics.text()).resolves.toContain("axiom_gateway_");
