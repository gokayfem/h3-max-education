...
import { cardInputSchema, sessionIdSchema } from "@/lib/server/session/schemas";
...
export async function POST(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const input = await parseJsonBody(request, cardInputSchema);
    const result = await createSessionServiceFromEnv().selectCard(learner, parsedSessionId.data, input);
